...
class GeminiService(AbstractLLMService):
        ...
        try:
            ...
            for msg in self.user_context:
                if msg["role"] == "user":
                    ...
                    # Gemini does NOT support role: "function"
                    # Instead, send function response as user message with function name
                    messages.append({
                        "role": "user",
                        "parts": [{
                            "function_response": {
    ...
    async def _handle_function_call(self, content, interaction_count):
        ...
        if function_name != "end_call":
            # Send function response back as user message with correct name
            await self.completion(function_response, interaction_count, role='function', name=function_name)
        return True
